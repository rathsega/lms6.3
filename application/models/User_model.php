...
class User_model extends CI_Model
{
    ...
    public function post_instructor_application($user_id = "", $document_custom_name ="")
    {
        if($user_id == ""){
        ...
        if ($user_details['email'] == $email) {
            ...
            if (isset($_FILES['document']) && $_FILES['document']['name'] != "") {
                ...
                if (in_array(strtolower($ext), $accepted_ext)) {
                    $document_custom_name = $document_custom_name == "" ? random(15) . '.' . $ext : $document_custom_name;
    ...
    function instructor_application(){
        ...
        if($user->num_rows() > 0){
            ...
            if ($previous_data == 0) {
                ...
                $data['document'] = $document_custom_name;
                move_uploaded_file($_FILES['document']['tmp_name'], 'uploads/document/' . $data['document']);
                $this->db->insert('applications', $data);
            }
        }
    }
    ...
    public function get_pending_applications()
    {
        ...
        $applications = $this->db->order_by('id', 'desc')->get('applications');
        return $applications;
    }
...
/*START LOGIN LOGOUT AND DEVICE ALLOW SECTION*/
   ...
   /* function update_unique_identifier($user_id = ""){
    ...
    //course-gift-ryan
    //course-gift-ryan
    public function upload_broucher_of_reviewer($image_code, $i)
    {
        if (isset($_FILES['broucher']) && $_FILES['broucher']['name'] != "" && $_FILES['broucher']['name'] != "") {
            move_uploaded_file($_FILES['broucher']['tmp_name'], 'uploads/broucher/' . $image_code );
            //$this->session->set_flashdata('flash_message', get_phrase('user_update_successfully'));
        }
    }

    public function get_user_image_url_of_custom_review($course_id, $offset)
    {
            return base_url() . 'uploads/user_image/placeholder.png';
    }
    
    public function get_broucher_url_of_custom_review($course_id)
    {
