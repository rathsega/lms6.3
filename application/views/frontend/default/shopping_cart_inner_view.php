...
<div class="col-lg-3 pt-1">
    ...
    <div class="cart-sidebar bg-white radius-10 py-4 px-3 box-shadow-5">
        ...
        <!-- send gift:-  -->
        <?php if(isset($gift_status) && $gift_status == 'gift'):?>
        <input type="checkbox" onclick="this.checked=!this.checked;" checked id="gift_checkbox">
        <label for="gift_checkbox"><strong><?php echo site_phrase('Send as a gift'); ?></strong></label>
        <div class="input-group mb-3" id="gift_email_input_field">
            <input type="text" class="form-control py-2" placeholder="<?php echo site_phrase('enter_the_gift_recepient\'s_email'); ?>" id="gift_email">
            
        </div>
        <?php else:?>
        <input type="checkbox" data-bs-toggle="collapse" data-bs-target="#gift_email_input_field" id="gift_checkbox">
        <label for="gift_checkbox"><strong><?php echo site_phrase('Send as a gift'); ?></strong></label>
        <div class="input-group mb-3 collapse" id="gift_email_input_field">
            <input type="text" class="form-control py-2" placeholder="<?php echo site_phrase('enter_the_recepient\'s_email'); ?>" id="gift_email">
            
        </div>
        <?php endif; ?>
        <div class="hidden" id= "no_user_found">
            <p class="text-danger ms-2"><b><?php echo get_phrase('recepient_must_be_an_existing_user').". ".get_phrase('no_user_found').".";?></b></p>
        </div>
        <div class="hidden" id= "email_empty">
            <p class="text-danger ms-2"><b><?php echo get_phrase('email_can\'t_be_empty');?></b></p>
        </div>
        <!-- send gift:-  end                 -->
        <button type="button" class="btn red w-100 radius-10 mb-3" onclick="checkGiftStatus()"><?php echo site_phrase('checkout'); ?></button>
    </div>
</div>