<?php foreach ($my_courses as $my_course):
  if(isset($my_course['course_id'])){
    ...
    $enrol_history = $this->crud_model->get_course_enrolled($my_course['course_id'], $this->session->userdata('user_id') )->row_array();
    $ratings_count = $this->crud_model->get_ratings_count($my_course['course_id']);
    if($ratings_count){
      ...
      $average_rating = ceil((($one*1) + ($two*2) + ($three*3)+($four*4)+($five*5))/$number_of_ratings);
    }else{
      $average_rating = 0;
    }
    
    if(date('Y-m-d') < date($enrol_history['expiry_date'])){
        $expired = false;
        $page = '/home/lesson/';
    }else{
        $expired = true;
...
<div class="col-sm-6 col-md-6 col-lg-4 col-xxl-3">
  <div class="course-box-wrap p-0">
    <div class="course-box p-0">
      <a href="<?php echo site_url($page.rawurlencode(slugify($course_details['title'])).'/'.$course_details['id']); ?>">
      ...
      <div class="pb-2" id = "course_info_view_<?php echo $course_details['id'];  ?>">
        <div class="course-details">
          ...
          <div class="rating your-rating-box text-start mt-1" style="position: unset;">
            ...
            <?php endif; ?>
            <?php endfor; ?>
            <a href="javascript:;" style="display:none;" id = "edit_rating_btn_<?php echo $course_details['id']; ?>" onclick="toggleRatingView('<?php echo $course_details['id']; ?>')" style="color: #2a303b; float: right; font-size: 13px;"><?php echo site_phrase('edit_rating'); ?></a>
        ...
        <div class="row">
          ...
          <a href="<?php $start = strtotime($enrol_history['expiry_date']); $end = strtotime(date('Y-m-d')); $days_between = ceil(abs($end - $start) / 86400); echo date('Y-m-d') < date($enrol_history['expiry_date']) ? site_url($page.rawurlencode(slugify($course_details['title'])).'/'.$course_details['id']) : site_url('home/course/'.rawurlencode(slugify($course_details['title'])).'/'.$course_details['id']); ?>" class="btn red radius-10 w-100"><?php echo date('Y-m-d') < date($enrol_history['expiry_date']) ? $days_between >= 60 ?  site_phrase('start_lesson') : "Expiring in $days_between days" : "Course Expired"; ?></a>
          </div>
        </div>
      </div>
