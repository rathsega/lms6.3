...
//<!gift-course-- ryan -->
    function checkGiftStatus(){
        var giftStatus = $('#gift_checkbox').is(':checked');
        if(giftStatus){
            var giftEmail = $("#gift_email").val();
            if(giftEmail !=""){

                url4 = '<?php echo site_url('home/gift_to_user_id'); ?>';
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var response = data;
                        //gift receiver user exists
                        if(response == 1){
                            $("#no_user_found").addClass("hidden");
                            $("#email_empty").addClass("hidden");
                            handleCheckOut();
                        } else {
                            $("#no_user_found").removeClass("hidden");
