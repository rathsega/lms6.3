...
if (addon_status('live-class')):
		...
		$live_class_end_time = $live_class->row('end_date');
		if ($live_class->num_rows() > 0 && strtotime($live_class_time) <= (time()+19800) && $live_class_end_time >= (time()+19800)):
...
<?php if($has_upcoming_live_class == 1): ?>
	...
	<?php echo get_phrase('live_class'); ?> <strong><?php echo date('h:i A', strtotime($live_class_time)) ." ". date('d M Y', time()+19800); ?></strong>.
	</div>
<?php endif; ?>